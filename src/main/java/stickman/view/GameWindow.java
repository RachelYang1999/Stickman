...
public class GameWindow {
  ...
  private void initialiseEntityViews() {
    ...
    List<Entity> staticEntitiesentities = model.getCurrentLevel().getStaticEntities();
    List<MovableEntity> dynamicEntities = model.getCurrentLevel().getDynamicEntities();

    for (Entity entity : staticEntitiesentities) {
  ...
  private void draw() {
    ...
    if (((GameEngineImpl) model).getNeedRestore()) {
      removeAll();
      initialiseEntityViews();
      ((GameEngineImpl) model).setNeedRestore();
    }
  }
}
